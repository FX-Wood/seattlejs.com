...
import { WebsiteSponsor, AirtablePhoto, WebsiteAirtablePair } from './repos/website-types.js';
...
*          and any new logos
*/
export const reconcileSponsors = (event: WebsiteAirtablePair,
    ...
    if (airtableEventSponsorIds) {
        for (let airtableId of airtableEventSponsorIds) {
            airtableEventSponsors.push(airtableSponsors.find(sponsor => sponsor.id == airtableId))
        }
    }
